...
async function AddFoodCategory(req, res) {
  ...
  try {
    if (!name || !file) {
      res
        .status(400)
        .json({ msg: "All fields are not provided", success: false });
    }
    ...
    if (response)
      return res
        .status(201)
        .json({ msg: "New category added", success: true, Category: response });
  } catch (error) {
    console.log(error);
    res
      .status(500)
      .json({ msg: "Error while adding food category", error, success: false });
  }
}
...
async function getAllFoodCategories(req, res) {
  try {
    ...
    console.log(error);
    res.status(500).json({
      msg: "Error while fetching all category",
      error,
      success: false
    });
  }
